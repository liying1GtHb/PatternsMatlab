function NCircIn(R,r,pos,rot,spd)
% Draw a fixed circle C0 centered at the origin, followed by a sequence of 
% circles Ci, all touching the previous one internally at (R,0), each 
% rolling clockwise along the previous circle's circumference. The program 
% traces the movement of the point pos initially located at P=(R-r(n)+pos,0)
...
p = plot(R-r(numCirc)+pos,0,'.','MarkerFaceColor','red');
%
...
for ii = 1:numSampt
    ...
    % trace the point pos;
    p.XData = lastxcenter(1:ii)+pos*cos(sum(alpha(:,1:ii),1)-sum(t(:,1:ii),1));
    p.YData = lastycenter(1:ii)+pos*sin(sum(alpha(:,1:ii),1)-sum(t(:,1:ii),1));
    drawnow;
end
end
